...
6. **Metodología Kanban Estricta en 4 Estados & Compuertas Bloqueantes (Stage Gates):**
   - Todo trabajo se gestiona bajo el modelo **Kanban de 4 Estados Obligatorios**: (`Por Hacer` ➔ `En Progreso` ➔ `En Revisión` ➔ `Completado`) en `projects/[active_project_id]/kanban-board.md`.
   - **Flujo de Revisión & Visto Bueno C-Level Obligatorio:** Ninguna tarea pasa directamente de `En Progreso` a `Completado`. Al concluir un entregable, el agente operativo ubica la tarea en **`En Revisión`** y notifica a su Líder Ejecutivo C-Level (`@ceo`, `@cmo`, `@cpo`, `@cto`, `@cfo`). El Líder C-Level audita la calidad técnica y estratégica del entregable, da su visto bueno oficial moviendo la tarea a `Completado (Done)` o solicita mejoras devolviéndola a `En Progreso` con feedback específico.
   - **Gobernanza y Auditoría por el Líder Kanban (@project-admin):** El agente `@project-admin` es el **Líder de la Metodología Kanban y Gobernanza del Repositorio**. Audita que los estados en `kanban-board.md`, las notas y la estructura de archivos se cumplan con absoluta rigurosidad antes del pase de estafeta.
   - **Protocolo de Compuertas Bloqueantes (Stage Gates):** Las fases de un proyecto son secuenciales y bloqueantes. Ningún agente puede iniciar tareas de fases posteriores si la compuerta previa no ha recibido el visto bueno C-Level y el estado `Completado`.
   - **Prohibición de Salto de Etapas:** Queda estrictamente prohibido a cualquier agente proponer o ejecutar tareas de fases futuras mientras existan compuertas o dependencias previas sin visto bueno oficial C-Level.
...
14. **Administración de Repositorio & Gobernanza de la Metodología Kanban (@project-admin):**
    - **Liderazgo de la Metodología Kanban:** El **Project & Repository Administrator** (`@project-admin`), bajo la supervisión del **CEO** (`@ceo`), es el **Líder de la Metodología Kanban y Gobernanza del Repositorio**. Audita que el tablero Kanban (`projects/[active_project_id]/kanban-board.md`), las compuertas de control y la estructura de archivos se cumplan con rigurosidad matemática.
...
16. **Protocolo Obligatorio de Entrega de Prompts con Gobernanza Kanban & Visto Bueno C-Level (Dual/Triple-Prompt Protocol):**
    - **Requisito Indispensable al Finalizar Respuesta:** Al concluir la ejecución de cualquier tarea o entregar un reporte, TODO agente DEBE incluir obligatoriamente al final de su mensaje de respuesta un bloque titulado `👉 PRÓXIMO PROMPT SUGERIDO PARA EL SIGUIENTE AGENTE` conteniendo **DOS O TRES PROMPTS CONSECUTIVOS**:
      - **PROMPT 1 (Verificación de Gobernanza Kanban por `@project-admin`):** Prompt listo para ejecutar dirigido al `@project-admin` (Líder Kanban) para auditar la persistencia del archivo en markdown, la actualización de `notes.md` y que la tarea esté correctamente en estado `En Revisión`.
      - **PROMPT 2 (Revisión & Visto Bueno por Líder C-Level):** Prompt listo para ejecutar dirigido al responsable C-Level (`@cmo`, `@cpo`, `@cto`, `@cfo`, `@ceo`) para revisar el entregable técnico/estratégico. Si el Líder C-Level aprueba, mueve la tarea a `Completado (Done)` y autoriza la ejecución del siguiente agente. Si encuentra fallas, devuelve la tarea a `En Progreso` con feedback específico.
      - **PROMPT 3 (Siguiente Agente Operativo):** Prompt listo para ejecutar del siguiente rol técnico u operativo desbloqueado (se ejecuta tras la aprobación C-Level).
