...
6. **Metodología Kanban & Cadena de Producción Secuencial Estricta (Stage Gates):**
   ...
   - **Gobernanza y Auditoría por el Líder Kanban (@project-admin):** El agente `@project-admin` es el **Líder de la Metodología Kanban y Gobernanza del Repositorio**. Audita que cada etapa se cumpla con absoluta rigurosidad antes del paso al siguiente agente.
...
14. **Administración de Repositorio & Gobernanza de la Metodología Kanban (@project-admin):**
    - **Liderazgo de la Metodología Kanban:** El **Project & Repository Administrator** (`@project-admin`), bajo la supervisión del **CEO** (`@ceo`), es el **Líder de la Metodología Kanban y Gobernanza del Repositorio**. Auditora que el tablero Kanban (`projects/[active_project_id]/kanban-board.md`), las compuertas de control y la estructura de archivos se cumplan con rigurosidad matemática.
...
16. **Protocolo Obligatorio de Entrega del Próximo Prompt con Verificación Kanban previa por @project-admin (Dual-Prompt Hand-off Protocol):**
    - **Requisito Indispensable al Finalizar Respuesta:** Al concluir la ejecución de cualquier tarea o entregar un reporte, TODO agente DEBE incluir obligatoriamente al final de su mensaje de respuesta un bloque titulado `👉 PRÓXIMO PROMPT SUGERIDO PARA EL SIGUIENTE AGENTE` conteniendo **DOS PROMPTS CONSECUTIVOS**:
      - **PROMPT 1 (Verificación de Gobernanza Kanban por `@project-admin`):** Prompt listo para copiar y pegar dirigido a `@project-admin` (Líder de Metodología Kanban) para que audite si el agente saliente cumplió con la metodología Kanban, persistió todos los entregables requeridos en `projects/[active_project_id]/`, actualizó correctamente `kanban-board.md` y `notes.md`, y respetó las directrices fundamentales.
        - *Regla de Modificación Remedial:* Si `@project-admin` detecta que la metodología Kanban NO se está respetando (ej. faltan entregables, notas desactualizadas, compuertas violadas), `@project-admin` DEBE modificar e incorporar instrucciones de corrección en el prompt del siguiente agente operativo antes de autorizar su ejecución. Si la metodología sí se respetó, `@project-admin` aprueba y da luz verde para ejecutar el PROMPT 2.
      - **PROMPT 2 (Siguiente Agente Operativo):** Prompt listo para copiar y pegar dirigido al siguiente agente operativo en la cadena de producción del Kanban (ej. `@seo-specialist`, `@ux-designer`, etc.).
